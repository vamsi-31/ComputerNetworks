...
The scripts are standalone demonstrations and are not currently integrated (e.g., CRC is not automatically used by the TCP scripts).

## Features / Contents

The project includes Python scripts demonstrating:

*   **TCP (Transmission Control Protocol) Scripts:** Simple client/server scripts for message and file transfer.
...
1.  Clone the repository:
    ...
    ```bash
    git clone https://github.com/vamsi-31/ComputerNetworks.git # Or your fork's URL
    cd ComputerNetworks
    ```
...
All scripts are run from the command line using Python within the project's root directory.
...
*   **Note on External Access for TCP/UDP:** If you intend to use these scripts over the internet (not just on your local network or machine), the receiver machine needs to be reachable. This typically involves configuring port forwarding on your router if the receiver is behind NAT, and using your public IP address or a DDNS service that points to your public IP. For local testing, `127.0.0.1` (localhost) is sufficient.
...
*   **`CheckSum/CheckSum_Sender.py`:** Prompts for a 100-bit binary string. Calculates and displays a 20-bit checksum and the data+checksum.
...
*   **`CheckSum/CheckSum_Reciver.py`:** Prompts for a 120-bit binary string (100 bits data + 20 bits checksum). Verifies the checksum for each segment.
...
*   **`Crc/Crc_Sender.py`:** Prompts for data bits and a generator polynomial (both binary strings). Shows the calculated CRC and the data to transmit.
...
*   **`Crc/Crc_Receiver.py`:** Prompts for received data (data + CRC) and the generator polynomial. Reports if errors are detected.

    ```bash
    python Crc/Crc_Receiver.py
    ```
    Follow the prompts (Use the "Transmitted Data" from the sender as input here):
    ```
...
*   **`Hamming/HammingSender.py`:** Prompts for data bits (binary string). Shows the generated Hamming code.
...
*   **`Hamming/Hamming_Receiver.py`:** Prompts for a received Hamming code (binary string). Reports if an error was found, the corrected code, and the extracted original data.

    ```bash
    python Hamming/Hamming_Receiver.py
    ```
    Follow the prompts (Use the Hamming code from the sender's output or introduce a single-bit error for testing):
    ```
    Enter received Hamming code: <Hamming_code_from_sender_or_with_an_error>
    ```
    Example (using output from sender for 1011 which might be 0110011 for a (7,4) Hamming code, then try changing one bit):
    ```bash
    python Hamming/Hamming_Receiver.py
